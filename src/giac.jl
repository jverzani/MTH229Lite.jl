...
function lambdify(ex)
    xs = Giac.free_symbols(ex)
    if isempty(xs)
        return x -> to_julia(ex)
    elseif length(xs) == 1
        return Giac.build_function(ex, only(xs))
    else
        error("Too many free symbols")
    end
end

function limit(ex::Giac.GiacInput, xc::Pair; dir="+")
    x, c = xc
    side = nothing
    if dir ∈ ("+", +, "right", :right)
        side = 1
    elseif dir ∈ ("-", -, "left", :left)
        side = -1
    end
    Giac.Commands.limit(ex, x, c, side)
end

## diff

## integrate
function integrate(ex::Giac.GiacInput, xab::Tuple)
    x, a, b = xab
    F = Giac.Commands.integrate(ex, x)
    F(b) - F(a)
end
