...
Once installed, each session the package must be loaded through the `using` command:
...
This pull in several helper functions and also loads several useful packages, such as `Roots`, `QuadGK`, `ForwardDiff`, and `SpecialFunctions`.

## Differences from the MTH229 package

`MTH229` also loads `SymPy` and anticipates the use of `Plots`.
...
The symbolic math package `SymPy` is not loaded, as it is with the `MTH229` package. The `SimpleExpressions` package is provided, which has some very limited symbolic support, but can still be useful.

## Functions and expressions

There is no additional support for Base `Julia` as regards the defining of functions.

However, the `SimpleExpressions` package allows an alternative for defining basic mathematical functions using expressions involving a symbolic value (and possible parameter). The following illustrates:
...
The object `u` is quite different from `f`, a function, in that it is a symbolic expression that can be subsequently manipulated. Its "call" method is overloaded so that `u(1)` substitutes in for `x`. When a parameter is used (the `p` in the `@symbolic` call) one can call `u(x,p)` to substitute in for both `x` and `p`; `u(x)` to substitute in for `x` (leaving `p`), or `u(:,p)` to substitute in for just `p` (leaving `x`).
...
This package provides an interface to specify intervals with `..` and can use symbolic expressions to specify the function. To illustrate, we have:
...
The `ForwardDiff` package is one of many for `Julia` for computing automatic derivatives. These have the advantage of being as exact as a symbolic derivative, but generally much faster to compute. The `MTH229Lite` package also overrides the `'` operator to take the derivative of a function:
...
The `SimpleExpressions` package also can take *simple* derivatives. The `'` notation is available:
...
* `find_zero(f, (a,b))` for `(a,b)` a **bracketing** interval will call a variable of bisection
* `find_zero(f, a)` for an initial guess `a` will call a methods similar to Newton's method (a hybrid secant method).

The `find_zeros(f, a, b)` scans over the intervals `[a,b]` and numerically attempts to identify all zeros.

When a function is *not* continuous, such as $f(x) = 1/x$ at $0$, these bracketing methods may return values where the function has a *sign* change.

Solving `f(x) = g(x)` is done by solving the related function `h(x) = f(x) - g(x) = 0`.

### Symbolic equations

The `SimpleExpressions` package provides a simple means to specify an equation with the `~` operator  (`=` is assignment, `==` is for relaxed comparison, `===` is for equality test).

The basic `plot` method is overridden to plot *both* functions; and `solve` has these variants:
...
An open box with square base is to be constructed from 108 inches of material. What dimensions produce the greatest volume?

This is a classic optimization problem which is solvable using techniques for a continuous scalar function over a closed interval. To get there, we identify the constraint: ``S = 108 = a^2 + 4\cdot a \cdot h``. The function to maximize is ``V= a \cdot a \cdot h``

We show how to solve this using functions

```@example lite
h(a) = (108 - a^2)/(4a) # solve the constraint for h
V(a) = a^2 * h(a)
```

Now, `a` can't be negative, and can't be bigger than ``\sqrt{108}``:
...
The answer is near `6`:

```@example lite
a = find_zero(V', 6)
```

In fact it is ``6``. Though a graph suffices, we can see from the first derivative test that we have a local maximum:
...
As with `MTH229`, a `riemann` function is provided for approximating definite integrals with  simple `Riemann` sums along with the trapezoid and Simpson's methods. The `quadgk` function from the `QuadGK` package is also provided for a more accurate and performant alternative.
...
* How much does this glass hold, in ml?
* How high to fill for ``16``oz (``473``ml)?
* How high to fill for  ``8``oz?

We will use `quadgk`. As that function returns a tuple of values and we only want the first, we use `first` below:
...
```@example lite
h(b, p) = V(b) - p
find_zero(h, (0, 15), 473)
```

And to fill half that, we get

```@example lite
find_zero(h, (0, 15), 473/2)
```
...
The plotting interface provided by `MTH229Lite` picks some of the common parts of `Plots.jl` and provides them for `PlotlyLight`, which otherwise is configured in a manner very-much like the underlying `JavaScript` implementation. The `Plost` package is great, but for resource-constrained usage can be too demanding.
...
The `plot(x,y)` function simply connects the pairs of points specified with a line in a dot-to-dot manner. If values in `y` are non finite, then a break in the dot-to-dot graph is made.

Related to `plot` and `plot!` is `scatter` (and `scatter!`) which plots just the points, but has no connecting dots.

For example, this shows how one could visualize the points chosen in a plot.
